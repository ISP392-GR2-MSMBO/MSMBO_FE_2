...
import { message, Spin, Button, Modal, Pagination } from "antd";
...
const UserManagement = () => {
    ...
    const validateUser = (user, isEditing = false) => {
        ...
        // @NotBlank checks
        if (!isEditing && !user.userName.trim()) errors.userName = "Tên đăng nhập không được để trống";
    ...
    // 🆕 Xử lý thay đổi Role (reset về trang 1 khi đổi tab)
    const handleRoleChange = (role) => {
        setViewRole(role);
        setCurrentPage(1);
    };

    // 🆕 Xử lý thay đổi tìm kiếm (reset về trang 1 khi tìm kiếm)
    const handleSearchChange = (e) => {
        setSearch(e.target.value);
        setCurrentPage(1);
    };

    // 🆕 Hàm xử lý phân trang và thay đổi pageSize (Giữ nguyên để tương thích với Pagination Ant Design)
    ...
    const handleSave = async (id) => {
        ...
        if (Object.keys(errors).length > 0) {
            const firstError = Object.values(errors)[0];
            messageApi.warning(`Lỗi khi lưu: ${firstError}`);
            return;
        }
    ...
    // --------------------------------------------------------
    // HÀM LỌC, TÌM KIẾM VÀ PHÂN TRANG
    // --------------------------------------------------------

    // 🔹 Bước 1: Lọc người dùng theo role & tìm kiếm
    const filteredAndSearchedUsers =
        users && users.length > 0
            ? users.filter((u) => {
                // Lọc theo Role
                let roleMatch = false;
                if (viewRole === "AD") roleMatch = u.roleID === "AD" || u.roleID === "MA";
                else if (viewRole === "CUS") roleMatch = u.roleID === "CUS";
                else if (viewRole === "ST") roleMatch = u.roleID === "ST";
                else roleMatch = true; // Bao gồm tất cả nếu viewRole không xác định
    ...
    // 🔹 Bước 2: Tính toán dữ liệu cho trang hiện tại
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    const paginatedUsers = filteredAndSearchedUsers.slice(startIndex, endIndex);
    ...
    return (
        <div className="user-management-container">
            ...
            <table className="user-table">
                ...
                <tbody>
                    {/* ✅ Hiển thị dữ liệu đã được phân trang */}
            ...
            {/* ✅ Thành phần Phân trang của Ant Design (Đã loại bỏ tùy chọn page size) */}
            <div style={{ marginTop: '20px', textAlign: 'right' }}>
                <Pagination
                    current={currentPage}
                    pageSize={pageSize}
                    total={filteredAndSearchedUsers.length} // Tổng số mục sau khi lọc và tìm kiếm
                    onChange={handlePageChange}
                // BỎ showSizeChanger, pageSizeOptions, và locale
                />
            </div>
